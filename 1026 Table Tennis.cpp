...
struct Table {
	...
	Table() : endTime(8 * 3600), vip(0), times(0) {}
};
struct Player {
...
int main() {
	...
	for (int i = 0, h, m, s, p; i < n; i++) {
		...
		if (newPlayer.arriveTime >= 21 * 3600) continue;
		newPlayer.playTime = p < 120 ? p * 60 : 7200;
		newPlayer.startTime = 21 * 3600;
	...
	while (curId < n) {
		...
		if (minTid == -1 || minEndTime >= 21 * 3600) break;  // if don't have a table or the earliest table ends after 21:00
	...
	for (int i = 0; i < n && players[i].startTime < 21 * 3600; i++) {
		...
		printf("%.0f\n", round(((double)players[i].startTime - (double)players[i].arriveTime) / 60.0));
	}
